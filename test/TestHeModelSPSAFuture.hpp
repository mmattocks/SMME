...
#include "ThreadRandomNumberGenerator.hpp"
...
public:
    ...
    void TestHeModelSPSAFixture()
    {
        ...
        p_log->Set(0, "HeModelParameterOptimise", "HeParamsTest");
        *p_log << "k\tphase2\tphase3\tPP1\tPP2\tPD2\tPP3\n";
        ...
        while (k < maxIterations)
        {
            ...
            *p_log << k << "\t" << thetaS(0) << "\t" << thetaS(1) << "\t" << thetaS(2) << "\t" << thetaS(3) << "\t"
            ...
            double AICgradient = evaluateAICgradient(thetaPlus,thetaMinus, false, HeModelParams, numberPoints);;

            vec ghat = ((AICgradient) / (2 * ck)) * delta;
    ...
    double evaluateAICgradient(vec thetaPlus, vec thetaMinus, bool detMode, unsigned numberParams, unsigned totalPoints)
    {
        //vectors to store the 3 residual sums of squares at each gradient sampling point
        vec RSSPlus = vec(3);
        vec RSSMinus = vec(3);

        //He et al. empirical probabilities for counts 1-1000
        vec probEmpirical24 = he24AICvec / lineagesSampled24;
        vec probEmpirical32 = he32AICvec / lineagesSampled32;
        vec probEmpirical48 = he48AICvec / lineagesSampled48;

        //Perform simulator runs as asynchronous tasks
        std::future<uvec> plus24 = std::async(std::launch::async, &TestHeModelSPSA::runSimulator, this, 24, thetaPlus, detMode);
        std::future<uvec> plus32 = std::async(std::launch::async, &TestHeModelSPSA::runSimulator, this, 32, thetaPlus, detMode);
        std::future<uvec> plus48 = std::async(std::launch::async, &TestHeModelSPSA::runSimulator, this, 48, thetaPlus, detMode);
        std::future<uvec> minus24 = std::async(std::launch::async, &TestHeModelSPSA::runSimulator, this, 24, thetaMinus, detMode);
        std::future<uvec> minus32 = std::async(std::launch::async, &TestHeModelSPSA::runSimulator, this, 32, thetaMinus, detMode);
        std::future<uvec> minus48 = std::async(std::launch::async, &TestHeModelSPSA::runSimulator, this, 48, thetaMinus, detMode);

        //wait for async tasks to return results
        plus24.wait();
        plus32.wait();
        plus48.wait();
        minus24.wait();
        ...
        //calculate residual sum  of squares for simulated data relative to empirical data
        RSSPlus(0) = sum(square((conv_to<vec>::from(plus24.get())/totalSeeds) - probEmpirical24));
        RSSPlus(1) = sum(square((conv_to<vec>::from(plus32.get())/totalSeeds) - probEmpirical32));
        RSSPlus(2) = sum(square((conv_to<vec>::from(plus48.get())/totalSeeds) - probEmpirical48));
        RSSMinus(0) = sum(square((conv_to<vec>::from(minus24.get())/totalSeeds) - probEmpirical24));
        RSSMinus(1) = sum(square((conv_to<vec>::from(minus32.get())/totalSeeds) - probEmpirical32));
        RSSMinus(2) = sum(square((conv_to<vec>::from(minus48.get())/totalSeeds) - probEmpirical48));

        double AICPlus = 2 * numberParams + totalPoints * log(sum(RSSPlus));
        double AICMinus = 2 * numberParams + totalPoints * log(sum(RSSMinus));
        double AICgrad = AICPlus - AICMinus;
    ...
    uvec runSimulator(double inductionTime, vec theta, bool deterministic)
    {
        ...
        ThreadRandomNumberGenerator* p_RNG = new ThreadRandomNumberGenerator;
