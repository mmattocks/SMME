...
class TestHeModelValidate : public AbstractCellBasedTestSuite
{
public:
    ...
    void TestHeModelValidateFixture()
    {
        Timer::Reset();
        Timer::Print("Begin TestHeModelValidateFixture @ ");
        ...
        //Count Writer Setup
        ColumnDataWriter* countWriter = SetupCountWriter("TestHeInductionCounts", "ValidateHeModelResults");

        //Event Writer Setup
        boost::shared_ptr<ColumnDataWriter> p_eventWriter(
                new ColumnDataWriter("HeModelMitoticRateEvents", "HeModelValidateEvents", false, 10));
        ...
        for (unsigned seed = startSeed; seed <= endSeed; seed++)
        {
            ...
            if (debugOutput)
            {
                ...
                debugWriter = SetupDebugOutput(p_cycle_model, inductionTime, "HeValidate", seed);
            }
            ...
            p_simulator->SetOutputDirectory("UnusedSimOutputValidateHeModelFixture");
            p_simulator->Solve();
    ...
    void TestDeterministicModelValidateFixture()
    {
        Timer::Reset();
        Timer::Print("Begin TestDeterministicModelValidateFixture @ ");
        ...
        //Count Writer Setup
        ColumnDataWriter* countWriter = SetupCountWriter("TestHeInductionCounts",
                                                            "ValidateDeterministicModelResults");

        //Event Writer Setup
        boost::shared_ptr<ColumnDataWriter> p_eventWriter(
                new ColumnDataWriter("HeModelMitoticRateEvents", "DeterministicModelMitoticRateEvents", false, 10));
        std::vector<int> eventWriterVarIDs = SetupEventWriter(p_eventWriter);
        ...
        for (unsigned seed = startSeed; seed <= endSeed; seed++)
        {
            ...
            {
                //Setup the writer and model for debug output
                debugWriter = SetupDebugOutput(p_cycle_model, inductionTime, "DeterministicValidate", seed);
            }
            ...
            p_simulator->SetDt(0.05);
            p_simulator->SetEndTime(endTime);
            p_simulator->SetOutputDirectory("UnusedSimOutputValidateDeterministicModelFixture");
            p_simulator->Solve();
    ...
    void TestShiftedDeterministicModelValidateFixture()
    {
        Timer::Reset();
        Timer::Print("Begin TestDeterministicModelValidateFixture @ ");
        ...
        //Count Writer Setup
        ColumnDataWriter* countWriter = SetupCountWriter("TestHeInductionCounts",
                                                            "ValidateShiftedDeterministicModelResults");

        //Event Writer Setup
        boost::shared_ptr<ColumnDataWriter> p_eventWriter(
                new ColumnDataWriter("HeModelMitoticRateEvents", "ShiftedDeterministicModelMitoticRateEvents", false,
                                        10));
        std::vector<int> eventWriterVarIDs = SetupEventWriter(p_eventWriter);
        ...
        for (unsigned seed = startSeed; seed <= endSeed; seed++)
        {
            ...
            {
                //Setup the writer and model for debug output
                debugWriter = SetupDebugOutput(p_cycle_model, inductionTime, "ShiftedDeterministicValidate", seed);
            }
            ...
            p_simulator->SetDt(0.05);
            p_simulator->SetEndTime(endTime);
            p_simulator->SetOutputDirectory("UnusedSimOutputShiftedValidateDeterministicModelFixture");
            p_simulator->Solve();
