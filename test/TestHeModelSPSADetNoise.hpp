...
public:
    ...
    void TestHeModelSPSADetFixture()
    {
        ...
        p_log->Set(0, "HeModelParameterOptimise", "DetNoise");
        *p_log << "k\tPhaseOffset\tb1Shape\tb1Scale\tb2Shape\tb2Scale\tphaseSisterShiftWidths\n";
        ...
        while (k < maxIterations)
        {
            ...
            *p_log << k << "\t" << thetaD(5) << "\t" << thetaD(0) << "\t" << thetaD(1) << "\t" << thetaD(2) << "\t"
                    << thetaD(3) << "\t" << thetaD(4) << "\n";

            AIC = evaluateAIC(thetaD, true, DModelParams, numberPoints, currStartSeed, currEndSeed, k);
