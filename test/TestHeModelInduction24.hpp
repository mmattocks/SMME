...
class TestHeModelInduction24 : public AbstractCellBasedTestSuite
{
public:
    ...
    const double inductionTime = 24.0;
    const std::string inductionTimeString = "24";
     ...
     **************************/
    //STOCHASTIC MITOTIC MODE (HE FIT)


    //STOCHASTIC MITOTIC MODE(REFIT)
    //mitotic mode per-phase probabilities
    //3-phase mitotic mode time periodisation
    const double mitoticModePhase2 = 4.94455;
    const double mitoticModePhase3 = 11.103;
    const double phase1PP = 0.999788;
    const double phase1PD = 0.0;
    const double phase2PP = 0.293016;
    const double phase2PD = 0.633621;
    const double phase3PP = 0.291788;
    const double phase3PD = 0.0;
    const unsigned HeModelParams = 15;
    //DETERMINISTIC MITOTIC MODE
    //Phase boundary shift parameters
    const double phaseSisterShiftWidths = 0.744407;
    const double b1Shape = 1.19708;
    const double b1Scale = 0.0001;
    const double b2Shape = 3.28225;
    const double b2Scale = 4.64745;
    const double phaseOffset = 5.9336;
    ...
    void TestHeInductionCount24Fixture()
    {
        Timer::Reset();
    ...
    void TestDeterministicModelInduction24Fixture()
    {
        Timer::Reset();
                          ...
                          bool deterministic = false, std::string debugDir = "debug")
    {
        ...
        else
        {
            ...
            double currPhase2Boundary = phase1Shift + p_RNG->GammaRandomDeviate(b1Shape, b1Scale);
            double currPhase3Boundary = currPhase2Boundary + p_RNG->GammaRandomDeviate(b2Shape, b2Scale);
